...
@Component({
  selector: 'ngx-ch-scale-pfeiffer',
  templateUrl: './ch-scale-pfeiffer.component.html',
  styleUrls: ['./ch-scale-pfeiffer.component.scss'],
})
export class ChScalePfeifferComponent implements OnInit {
  @ViewChild(BaseTableComponent) table: BaseTableComponent;
  @Input() refresh22;

  linearMode = true;
  public messageError = null;
  public title: string = 'Registros Escala Pfeiffer';
  ...
  ngOnChanges(changes: SimpleChanges) {
    if(changes.refresh22.currentValue==true){
      this.RefreshData();
    }
  ...
  Historic() {
    this.dialogFormService.open(ChHistoricScalesComponent, {
      context: {
        title: 'Escala Pfeiffer',
        path: 'ch_scale_pfeiffer',
        catch_info: 'ch_scale_pfeiffer',
