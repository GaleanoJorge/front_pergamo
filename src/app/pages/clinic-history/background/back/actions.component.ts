...
<ng-template #confirmAction>
  <div class="container-fluid" fullWidth>
    ...
    <nb-card-header>Edición Antecedentes: {{this.value.data.ch_type_background.name}}</nb-card-header>
    <nb-card-body>
    <form [formGroup]="form" (ngSubmit)="EditBackground()">
  <div class="row">


  <div class="col-md-12">
    <nb-radio-group [disabled]="this.disabled" formControlName="revision" revision id="revision" class="d-flex text" name="revision"
      status="{{ isSubmitted && form.controls.revision.errors ? 'danger' : isSubmitted ? 'success' : 'basic' }}">
      <nb-radio value="SI">SI</nb-radio>
      <nb-radio value="NO REFIERE">NO REFIERE</nb-radio>
    </nb-radio-group>
  </div>
