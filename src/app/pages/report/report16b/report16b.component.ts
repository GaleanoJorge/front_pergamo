...
@Component({
  selector: 'ngx-report16b',
  templateUrl: './report16b.component.html',
  styleUrls: ['./report16b.component.scss']
})
export class Report16bComponent implements OnInit {
  ...
  Search() {
    ...
    this.reporteBS.GetReport16B({
      ...
      this.data.labels = ['CriteriosAprobados', 'CriteriosNoAprobados'];
      this.data.series = [];
      x.data.report.forEach(element => {
        this.data.series.push({
          value: element['CriteriosAprobados'],
          name: 'CriteriosAprobados',
        })
        this.data.series.push({
          value: element['CriteriosNoAprobados'],
          name: 'CriteriosNoAprobados',
        })
      });
    }).catch(x => {
      this.messageError = null;
