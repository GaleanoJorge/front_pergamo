...
export class ServerDataSourceCustom extends ServerDataSource {
  ...
  constructor(protected http: HttpClient, entity: string,
              customData = null, conf: ServerSourceConf | {} | any = {}, prefix: string = 'data.', queryParams = {}) {
    super(http, ((!conf.endPoint ?
      (conf.endPoint = environment.api + entity) : conf.endPoint = conf.endPoint),
      (!conf.dataKey ?
        (conf.dataKey = prefix + (customData ? customData : entity) + '.data') : conf.dataKey = conf.dataKey),
      (!conf.pagerLimitKey ?
        (conf.pagerLimitKey = prefix + (customData ? customData : entity) + '.per_page')
        : conf.pagerLimitKey = conf.pagerLimitKey),
      (!conf.pagerPageKey ?
        (conf.pagerPageKey = prefix + (customData ? customData : entity) + '.current_page')
        : conf.pagerPageKey = conf.pagerPageKey),
      (!conf.totalKey ?
        (conf.totalKey = prefix + (customData ? customData : entity) + '.total') : conf.totalKey = conf.totalKey),
      conf),
    );
    this.entity = entity;
    this.prefix = prefix;
  ...
  protected addPagerRequestParams(httpParams: HttpParams): HttpParams {
    ...
    if (this.pagingConf && this.pagingConf['page'] && this.pagingConf['perPage']) {
      const pagerKey = this.conf.pagerPageKey.replace(this.prefix +
        (this.customData ? this.customData : this.entity) + '.', '');
      const pagerLimitKey = this.conf.pagerLimitKey.replace(this.prefix +
        (this.customData ? this.customData : this.entity) + '.', '');
