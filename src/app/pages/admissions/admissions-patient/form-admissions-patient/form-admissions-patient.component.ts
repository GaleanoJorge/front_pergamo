...
export class FormAdmissionsPatientComponent implements OnInit {
  ...
  admissionRouteChanged(val) {
    ...
    if (val === '') {
      ...
      if (val == 1) {
        this.form.controls.procedure_id.setValidators(
          Validators.compose([Validators.required])
        );
  ...
  GetBed(pavilion_id, ambit) {
    ...
    return this.BedS.GetBedByPavilion(
      pavilion_id,
      ambit,
      this.ambolatory ? { office: this.data.medical_diary.office_id } : {}
    ).then((x) => {
      this.bed = x;
