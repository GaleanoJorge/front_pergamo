...
import { TcQuitService } from '../../../business-controller/tc-quit.service';

@Component({
  selector: 'ngx-tc-quit',
  templateUrl: './tc-quit.component.html',
  styleUrls: ['./tc-quit.component.scss']
})
export class TcQuitComponent implements OnInit {
  ...
  public routes = [
    {
      name: 'Abandonos',
      route: '../../setting/tc-quit',
    },
  ];

  constructor(
    private TcQuitS: TcQuitService,
  ...
  async uploadDocumentInfo(lectura) {
    try {
      let response;
      response = await this.TcQuitS.SaveFile(lectura);
      this.loading2 = false;
