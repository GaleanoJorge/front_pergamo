...
export class ItemRolePermissionBusinessService {
  ...
  constructor(private webAPI: WebAPIService, private dbpouch: DbPwaService) {
  ...
  GetCollection(idRole?: number, user?): Promise<ItemRolePermission[]> {
    ...
    if (navigator.onLine) {
      return this.webAPI
        ...
        .then((x) => {
          ...
          this.permission.forEach((element) => {
            if (
              !element.item.item_parent_id &&
              element.permission_id === 1 &&
              element.item.show_menu
            ) {
              mainMenu.push({
                id: element.item.id,
              ...
              mainMenu.forEach((elementB) => {
                if (
                  elementB.id === element.item.item_parent_id &&
                  element.permission_id === 1 &&
                  element.item.show_menu
                ) {
                  if (
                    elementB.children.findIndex(
                      (x) => x.id === element.item.id
                    ) === -1
                  ) {
            ...
            if (elementC.children.length != 0) {
              elementC.children.forEach((elementD) => {
                servObj.data.itemRolePermission.forEach((element) => {
                  if (
                    elementD.id === element.item.item_parent_id &&
                    element.permission_id === 1 &&
                    element.item.show_menu
                  ) {
                    if (
                      elementD.children.findIndex(
                        (x) => x.id === element.item.id
                      ) === -1
                    ) {
                  ...
                  elementF.children.forEach((elementG) => {
                    if (elementG.children) {
                      servObj.data.itemRolePermission.forEach((elementH) => {
                        if (
                          elementG.id === elementH.item.item_parent_id &&
                          elementH.permission_id === 1 &&
                          elementH.item.show_menu
                        ) {
                          if (
                            elementG.children.findIndex(
                              (x) => x.id === elementH.item.id
                            ) === -1
                          ) {
      ...
      this.user_db.get(user).then((x) => {
        localStorage.setItem('mainMenu', JSON.stringify(x.menu));
        localStorage.setItem('firstMenu', JSON.stringify(x.menu[0]));
